(function () {
  ...
  const SERVICES = {
    "Corte": { price: 220, duration: 60, image: "assets/images/trabajos/trabajo-18.jpeg", description: "Corte de precisión y peinado final." },
    "Corte de niño": { price: 220, duration: 40, image: "assets/images/trabajos/trabajo-08.jpeg", description: "Corte infantil con detalle y comodidad." },
    "Corte y Barba": { price: 330, duration: 60, image: "assets/images/trabajos/trabajo-36.jpeg", description: "Corte completo con perfilado de barba." },
    ...
    "Facial": { price: 180, duration: 60, image: "assets/images/facial.jpeg", description: "Rutina facial para limpiar y revitalizar." },
    "Grecas": { price: 50, duration: 40, image: "assets/images/trabajos/trabajo-21.jpeg", description: "Diseño de líneas y detalles personalizados." },
    "Paquete Luxury VIP": { price: 500, duration: 60, image: "assets/images/paquete-luxury-vip.jpeg", description: "Corte, barba, ceja y facial." },
    "Paquete Luxury": { price: 380, duration: 60, image: "assets/images/paquete-luxury.png", description: "Corte, ceja y facial." }
  };
  ...
  function renderSlots(slots) {
    ...
    if (!available.length) {
      slotsBox.innerHTML = '<p class="slots-empty">Ese día ya no tiene horarios disponibles. Elige otra fecha.</p>';
      return;
    }

    slotsBox.innerHTML = slots.map((slot) => {
      const selected = state.time === slot.time;
      // El cliente no ve cuántos espacios internos quedan.
      // Mientras exista al menos uno de los dos lugares, solo se muestra Disponible.
      const availabilityText = slot.available ? "Disponible" : "Reservado";
      return `
  ...
  function qrSafeValue(value, maxLength) {
    return String(value ?? "")
      .replace(/[\r\n|]+/g, " ")
      .replace(/\s+/g, " ")
      .trim()
  ...
  function qrText(booking) {
    // Mantener el QR pequeño y compatible con qrcode.js.
    // La ficha PDF conserva el resto de la información visualmente.
    const payload = [
      "TLS",
      `F:${qrSafeValue(booking.folio, 45)}`,
      `N:${qrSafeValue(booking.nombre, 70)}`,
      `T:${qrSafeValue(booking.telefono, 25)}`,
      `S:${qrSafeValue(booking.servicio, 80)}`,
      `D:${qrSafeValue(booking.fecha, 10)}`,
      `H:${qrSafeValue(booking.hora, 5)}-${qrSafeValue(booking.horaFin, 5)}`
    ].join("|");

    // Protección adicional: nunca permitir un payload enorme.
    ...
    if (qrNode) {
      ...
      if (window.QRCode) {
        try {
          ...
          // Guardar exactamente el mismo bitmap que se muestra al usuario.
          state.lastQrDataUrl = getQrDataUrl();
        } catch (error) {
          state.lastQrDataUrl = "";
  ...
  function loadCanvasImage(src) {
    return new Promise((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error("No fue posible preparar el código QR."));
      image.src = src;
    });
  }
  ...
  function createSinglePagePdfFromJpeg(jpegBytes, imageWidth, imageHeight) {
    const pageWidth = 419.53; // A5 en puntos PDF
  ...
  async function buildTicketCanvas(booking) {
    // A5 vertical a resolución suficiente para impresión nítida.
    const canvas = document.createElement("canvas");
    canvas.width = 1240;
    canvas.height = 1754;
    const ctx = canvas.getContext("2d");
    if (!ctx) throw new Error("Tu navegador no pudo crear la ficha PDF.");
    ...
    ctx.fillText("THE LUXURY STUDIO", margin, 112);
    ctx.fillStyle = cream;
    ctx.font = "700 76px Georgia, serif";
    ctx.fillText("Cita confirmada", margin, 215);
    ctx.fillStyle = "#c8c0b1";
    ctx.font = "400 26px Arial, sans-serif";
    ctx.fillText(`Folio ${booking.folio || "—"}`, margin, 282);
    ...
    const prettyDate = booking.fecha
      ? new Date(`${booking.fecha}T12:00:00`).toLocaleDateString("es-MX", {
          weekday: "long",
          day: "numeric",
          month: "long",
          year: "numeric"
    ...
    const rows = [
      ...
      ["TOTAL", formatPrice(Number(booking.precio || 0))],
    ...
    rows.forEach(([label, value]) => {
      ...
      ctx.fillText(label, cardX + 46, y);

      ctx.fillStyle = ink;
      ctx.font = "600 28px Arial, sans-serif";
      const valueX = cardX + 300;
      const lines = wrapCanvasText(ctx, String(value), cardW - 350).slice(0, 2);
      lines.forEach((line, index) => ctx.fillText(line, valueX, y + index * 34));

      y += lines.length > 1 ? 92 : 76;
      ctx.strokeStyle = "#e8e1d5";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(cardX + 46, y - 36);
      ctx.lineTo(cardX + cardW - 46, y - 36);
      ctx.stroke();
    });

    // QR.
    // Usar exactamente el QR que se mostró en la pantalla de confirmación.
    const qrData = state.lastQrDataUrl || getQrDataUrl();
    const qrImage = await loadCanvasImage(qrData).catch(() => null);
    const qrSize = 380;
    const qrX = (W - qrSize) / 2;
    const qrY = 1135;

    ctx.fillStyle = "#ffffff";
    canvasRoundRect(ctx, qrX - 28, qrY - 28, qrSize + 56, qrSize + 56, 24);
    ctx.fill();
    ...
    if (qrImage) {
      ...
      ctx.fillText("QR no disponible", W / 2, qrY + qrSize / 2);
      ctx.textAlign = "left";
    }
    ...
    ctx.fillText("PRESENTA ESTE QR AL LLEGAR", W / 2, 1588);
    ctx.fillStyle = muted;
    ctx.font = "400 21px Arial, sans-serif";
    ctx.fillText("Av. Repúblicas 94, Portales Sur, Benito Juárez, CDMX", W / 2, 1634);
    ctx.fillStyle = gold;
    ctx.font = "700 18px Arial, sans-serif";
    ctx.fillText("THE LUXURY STUDIO", W / 2, 1687);
    ctx.textAlign = "left";
  ...
  async function downloadTicket() {
    ...
    if (downloadButton) {
      ...
      downloadButton.dataset.originalText = downloadButton.textContent || "Descargar ficha PDF ↓";
      downloadButton.textContent = "Generando PDF…";
    }

    try {
      // La ficha se convierte en una sola imagen A5 y después en un PDF
      // de UNA ÚNICA PÁGINA. No se usa window.print().
      const canvas = await buildTicketCanvas(booking);
      const jpegDataUrl = canvas.toDataURL("image/jpeg", 0.94);
      const jpegBytes = dataUrlToBytes(jpegDataUrl);
      const pdfBlob = createSinglePagePdfFromJpeg(jpegBytes, canvas.width, canvas.height);
      const objectUrl = URL.createObjectURL(pdfBlob);
      const link = document.createElement("a");
      link.href = objectUrl;
      link.download = `Cita-${booking.folio || "Luxury"}.pdf`;
      ...
      showModalStatus(error.message || "No fue posible generar la ficha PDF.", "error");
    } finally {
      if (downloadButton) {
        downloadButton.disabled = false;
        downloadButton.textContent = downloadButton.dataset.originalText || "Descargar ficha PDF ↓";
      }
    }
  }
